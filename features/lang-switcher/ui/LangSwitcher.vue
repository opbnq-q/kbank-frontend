...
<template>
    <div class="relative w-[100px] flex items-center justify-center">
        <CommonBaseButton :class="['w-full', {'scale-50': isOpened}]" @click="isOpened = true">{{ locales.find(el => el.code == locale)?.name }}</CommonBaseButton>
        <section
            ...
            <CommonBaseButton class="w-20" v-for="l in locales" :key="l.code" @click="handleClick(l.code)">
                {{ l.name }}
            </CommonBaseButton>
        </section>
    </div>
